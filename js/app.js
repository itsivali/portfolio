...
function loadTheme(theme) {
  ...
  if (theme === "light") {
    themeBtn.innerHTML = `<em style="background-color: white; padding: 5px; border-radius: 50%;"></em><i class="ph-bold ph-moon-stars"></i>`;
    faqItems.forEach(item => {
      item.style.backgroundColor = 'white';
      item.style.color = '#000'; 
    });
  } else {
    themeBtn.innerHTML = `<em style="background-color: white; padding: 5px; border-radius: 50%;"></em><i class="ph-bold ph-sun"></i>`;
    faqItems.forEach(item => {
      item.style.backgroundColor = '#000';
      item.style.color = '#c7c6d3'; 
    });
  }
